...
def plot_category_summary(summary: pd.DataFrame) -> None:
    ...
    profits = summary["预测经营利润_元"].to_numpy(dtype=float)
    category_names = summary["分类名称"].astype(str).tolist()
    ...
    # 所有柱子位于同一纵深位置
    ypos = np.zeros(n, dtype=float)

    # 柱体宽度和厚度
    dx = np.full(n, 0.58, dtype=float)
    dy = np.full(n, 0.68, dtype=float)
    ...
    max_profit = max(
        np.max(np.abs(profits)),
        1
    )

    label_offset = max_profit * 0.035
    ...
    try:
        ...
        ax2.xaxis.pane.set_facecolor(
            "#F5F3EF"
        )

        ax2.yaxis.pane.set_facecolor(
            "#F8F6F2"
        )

        ax2.zaxis.pane.set_facecolor(
            "#FAF9F7"
        )


        ax2.xaxis.pane.set_alpha(
            0.15
        )

        ax2.yaxis.pane.set_alpha(
            0.08
        )

        ax2.zaxis.pane.set_alpha(
            0.06
        )
    ...
    try:
        ...
        ax2.xaxis.line.set_color(
            "#999999"
        )

        ax2.yaxis.line.set_color(
            "#BBBBBB"
        )

        ax2.zaxis.line.set_color(
            "#999999"
        )
