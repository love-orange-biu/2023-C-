...
def write_model_note(
    ...
    (OUTPUT_DIR / "第四问_建模说明.md").write_text("\n".join(lines), encoding="utf-8")
...
def write_excel(
    ...
    path = OUTPUT_DIR / "第四问_数据采集建议.xlsx"
    priority_columns = [
        "综合排名",
        "数据主题",
        "实施批次",
...
def main() -> None:
    ...
    excel_path = write_excel(priority_df, impact_df, inventory_df, notes_df, sensitivity_df)
    plot_priority(priority_df)
    plot_impact_matrix(priority_df)
    plot_data_loop()
    write_model_note(priority_df, impact_df, row_count_map)
