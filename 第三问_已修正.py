...
def plot_category_summary(summary: pd.DataFrame) -> None:
    """绘制品类需求满足哑铃图和品类预测经营利润图。"""

    # -----------------------------------------------------------------
    # 直接使用 solve_three_stage() 生成的 summary 真实列名
    # -----------------------------------------------------------------
    categories = summary["分类名称"].astype(str).to_numpy()
    target_demand = pd.to_numeric(
        summary["品类目标需求_kg"], errors="coerce"
    ).to_numpy(float)
    optimized_sales = pd.to_numeric(
        summary["预测销量_kg"], errors="coerce"
    ).to_numpy(float)
    satisfaction = pd.to_numeric(
        summary["需求满足率"], errors="coerce"
    ).to_numpy(float)

    if (
        np.isnan(target_demand).any()
        or np.isnan(optimized_sales).any()
        or np.isnan(satisfaction).any()
    ):
        raise ValueError("品类汇总中存在无法转换为数值的目标需求、预测销量或需求满足率。")

    # =================================================================
    # 图 2-1：各品类市场需求满足情况（哑铃图）
    # =================================================================
    ...
    for i in range(len(categories)):
        ax1.plot(
            [optimized_sales[i], target_demand[i]],
            [y[i], y[i]],
            color=line_color,
            linewidth=3.0,
            alpha=0.70,
    ...
    # 满足率标注
    max_x = float(np.nanmax(target_demand))
    for i, rate in enumerate(satisfaction):
        ax1.text(
            max(target_demand[i], optimized_sales[i]) + max_x * 0.014,
            y[i],
            f"{rate:.1%}",
            va="center",
            ha="left",
    ...
    ax1.set_yticks(y, categories)
    ax1.set_xlabel("需求量 / 预测销量（千克）", fontsize=13)
    ax1.set_ylabel("蔬菜品类", fontsize=13)
    ax1.set_title("各品类市场需求满足情况", fontsize=18, pad=14)
    ax1.invert_yaxis()
    ax1.set_xlim(0, max_x * 1.12)
    ax1.grid(axis="x", color="#D8D8D8", linestyle="--", alpha=0.30)
    ax1.set_axisbelow(True)
    ...
    colors = [
        CATEGORY_COLORS.get(str(c), "#8FAED3")
        for c in summary["分类名称"]
    ]
    profit_values = pd.to_numeric(
        summary["预测经营利润_元"], errors="coerce"
    ).to_numpy(float)
    ...
    # 正利润标签放柱顶，负利润标签放柱底
    y_range = max(
        float(np.nanmax(profit_values) - np.nanmin(profit_values)),
        1.0,
    )
