...
def plot_category_summary(summary: pd.DataFrame) -> None:
    """绘制品类需求满足折线图和品类预测经营利润图。"""

    categories = summary["分类名称"].astype(str).tolist()
    ...
    if (
        ...
        raise ValueError("品类汇总中存在无法转换为数值的列。")

    # ================================================================
    # 图2-1：各品类市场需求满足情况（双折线图）
    # ================================================================
    x = np.arange(len(categories))
    fig1, ax1 = plt.subplots(figsize=(11, 6.5), facecolor="white")

    target_color = "#C9CDD3"   # 低饱和灰蓝
    sales_color = "#8FAED3"    # 低饱和蓝色
    ...
    for i in range(len(categories)):
        ymax = max(target_demand[i], optimized_sales[i])
        ax1.text(
            x[i],
            ymax + max(target_demand) * 0.025,
            f"{satisfaction[i]:.1%}",
            ha="center",
            va="bottom",
            fontsize=11,
    ...
    ax1.set_ylabel("销量（千克）", fontsize=13)
    ax1.set_xlabel("蔬菜品类", fontsize=13)
